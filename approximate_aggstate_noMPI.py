...
class approximate(object):
    ...
    def integrate(self,f):
        ...
        def f_int(x):
            z,w = x
            return w*f(z)
        return sum(map(f_int,self.dist))
    ...
    def iterate(self,Zbar,quadratic = True):
        ...
        Y1hat = sum(map(lambda z : self.dY(z[1]).dot((z[0]-z[1])),Gamma_dist))/len(self.Gamma)
        def Y2_int(x):
            z_i,zbar = x
            zhat = z_i-zbar
            return (quadratic_dot(self.d2Y[z,z](zbar),zhat,zhat) + 2* quadratic_dot(self.d2Y[z,Y](zbar),zhat,Y1hat)).flatten()
        
        Y2hat = sum(map(Y2_int,Gamma_dist))/len(self.Gamma)
        
        def Y2_GZ_int(x):
            z_i,zbar = x
            zhat = z_i-zbar
            return quadratic_dot(self.d2Y[z,Z](zbar),zhat,np.eye(nZ)).reshape(nY,nZ)
        Y2hat_GZ =  sum(map(Y2_GZ_int,Gamma_dist))/len(self.Gamma)
        ...
        ye = np.vstack(map(compute_ye,Gamma_dist))
    ...
    def iterate_ConditionalMean(self,Zbar):
        ...
        Y1hat = sum(map(lambda z : self.dY(z[1]).dot((z[0]-z[1]))
